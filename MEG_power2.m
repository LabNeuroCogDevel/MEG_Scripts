function [ pow ] = MEG_power2( tfr, baseline, trial, trialsN )
...
%and baseline in percent change. This function takes the 10log10 first for
%trial by trial power estimates,then convert to signal change using trial
%specific baseline.
%
% Usage: [ pow ] = MEG_power2( tfr, baseline, trial, trialsN )
...
if isempty(trialsN)
    pow.singletrial_powspctrm = 10.*log10(tfr.powspctrm(:,:,:,tstart:tend));
    baseline_mean = nanmean(10.*log10(tfr.powspctrm(:,:,:,bstart:bend)),4);
    baseline_mean = nanmean(baseline_mean);
    b_mean=repmat(baseline_mean,[size(tfr.powspctrm,1),1,1,size(pow.singletrial_powspctrm,4)]);
   % b_mean = repmat(baseline_mean, [size(tfr.powspctrm,1),size(tfr.powspctrm,2),size(tfr.powspctrm,3), tend-tstart+1]);
    ...
    pow.singletrial_powspctrm = 10.*log10(tfr.powspctrm(trialsN,:,:,tstart:tend));
    baseline_mean = nanmean(10.*log10(tfr.powspctrm(trialsN,:,:,bstart:bend)),4);
    baseline_mean = nanmean(baseline_mean);
    b_mean=repmat(baseline_mean,[size(pow.singletrial_powspctrm,1),1,1,size(pow.singletrial_powspctrm,4)]);
