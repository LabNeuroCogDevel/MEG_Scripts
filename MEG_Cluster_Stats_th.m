function [ Stats, Clusters, Clust_Masks, Sig_Mask, Clust_Pvals, Sig_Pvals, Null_clusts_mass ] = MEG_Cluster_Stats_th( Data1, Data2, nPerm, th)
...
%Null distribution. You would have to input the corrected cluster level
%threshold.
%For details see As described in Maris and Oostenveld et
%al 2007.
%
%   Usage: [ Stats, Clusters, Clust_Masks, Sig_Mask, Clust_Pvals,
%   Sig_Pvals, Null_clusts_mass ] = MEG_Cluster_Stats( Data1, Data2, nPerm, th)
%
%   Input:
...
%
%   th is the cluster level threshold.
%   
%   Output:
...
%find critical test stat value at 97.5 percentile for two tail test.
...
if any(Clust_Pvals < th)
    a = find(Clust_Pvals<th);
    for i = 1:length(a)
        tempMask = test_clusts==a(i);
        Sig_Mask = Sig_Mask + tempMask;
    end
    Sig_Pvals = Clust_Pvals(Clust_Pvals < th);
else
    Sig_Pvals = [];
end
